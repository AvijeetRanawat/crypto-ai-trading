...
def reset_portfolio():
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?)
    """, (timestamp, 1250.0, 0))
    print(f'✓ Created new portfolio snapshot with $1250.00 balance')
    
    # Optional: Clear signal events (comment out if you want to keep these)
    cur.execute("DELETE FROM signal_events")
    signals_deleted = cur.rowcount
    print(f'✓ Deleted {signals_deleted} signal events')
    
    # Optional: Clear RL events (comment out if you want to keep these)
